...
<template>
  <div class="flex flex-col gap-[20vh] md:mt-[27vh] mt-[10vh] w-full">
    <section class="flex gap-x-9 relative" v-if="article">
      <div class="flex-1">
        <div class="flex flex-row items-center md:hidden mb-3">
          <h4 :key="article.title" class="text-xl font-semibold tracking-wider">
            {{ article.title }}
          </h4>

          <button class="hidden btn btn-ghost rounded-none ml-auto">
            <svg
      ...
      <div class="max-w-[34.7%] w-full md:block hidden" :key="article.title">
        <div class="flex flex-col sticky top-[104px]">
          ...
          >
            <span class="uppercase mb-2 font-myriad-light"
              >FUNCTION:
              <span class="ml-1">{{ article.functions.join(", ") }}</span></span
            >
          </InlineEditor>
